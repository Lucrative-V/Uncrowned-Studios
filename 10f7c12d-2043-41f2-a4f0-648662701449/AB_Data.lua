...
local module = {
	...
	FrameData = {
		...
		Movement = 'N/A'; -- Describes the general thrust direction that the move will tend to follow 
	...
	HitboxData = {
		['Hitbox_1'] = {				
			DelayTime = 0;
			Duration = 1000; 
			Size = Vector3.new(10, 10, 50);
			RelativeToOwner = true;
			FixedCoordinates = CFrame.new(0,0,-25);
			FixedToOwner = false;
			DamageInterval = 1; 
			RepeatCount = nil; 
			...
			HitDamage = 7.5; -- Damage inflicted on target directly
	...
	local function Startup()
		...
		local function StartHitbox()
			...
			local function GenerateProxyHitbox(insertData)
				...
				_G.EffectService:FireAllClients('AirBlast','AirBlastWithProxy', {
					Character = character;
		...
		task.delay(selectedHitboxData.DelayTime or 0, function()
			...
			if player then 
				playerData.Repository.NetworkManager:FireClient(player, 'ToolInputData' , {
					...
					Duration = (module.FrameData.StartUp/FRAME_RATE) + (module.FrameData.Active/FRAME_RATE) 
				})
			end
...
function module:ExecuteAbility_Client(options)
	...
	local function characterStatis(options)
		...
		for i = 0, launchDuration or .1, 1/60 do 
			...
			launchVelocity.Velocity = Vector3.zero
