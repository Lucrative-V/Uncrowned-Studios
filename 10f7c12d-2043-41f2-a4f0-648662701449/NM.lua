--[[
	READ ME [ Forked by : Lucrative#2824 { Includes the ability to utilize bindables } ]
	...
	https://devforum.roblox.com/t/easynetwork-creates-remote-functionsevents-for-you-so-you-dont-have-to/571258
--]]
...
local BindableEventHandlers = {}
local BindableFunctionHandlers = {}
...
function SafeBindableInvoke(timeout, handler, ...)
	local thread = coroutine.running()
	local finished = false
	local result

	coroutine.wrap(function(...)
		result = table.pack(pcall(function(...) return handler.Bindable:Invoke(...) end, ...))
...
function GetBindableEventHandler(name)
	local handler = BindableEventHandlers[name]
	if handler then
		return handler
	end
	...
	BindableEventHandlers[name] = handler

	local bindable = Instance.new("BindableEvent")
	bindable.Name = handler.Name
	bindable.Parent = handler.Folder

	handler.Bindable = bindable
		...
		bindable.Event:Connect(function(...)
			if handler.IncomingQueue then
				if #handler.IncomingQueue >= 2048 then
					if not handler.IncomingQueueErrored then
						handler.IncomingQueueErrored = true
						FastSpawn(error, string.format("Exhausted remote invocation queue for %s", bindable:GetFullName()), -1)
...
function GetBindableFunctionHandler(name)
	local handler = BindableFunctionHandlers[name]
	if handler then
		return handler
	end
	...
	BindableFunctionHandlers[name] = handler
...
function AddBindableToQueue(handler, fn, doWarn)
	if handler.Bindable then
		return fn()
	end
...
function Network:SynchronizeEvents(pre, callbacks)
	if typeof(pre) == "table" then
		pre, callbacks = nil, pre
	end

	for name,fn in pairs(callbacks) do
		local handler = GetBindableEventHandler(name)
		if not handler then
			error(("Tried to bind callback to non-existing BindableEvent %q"):format(name))
		end

		handler.Callbacks[#handler.Callbacks + 1] = combineFn(handler, fn, pre)
		
		handler.Bindable.Event:Connect(function(...)
			SafeFireEvent(handler, ...)
		end)
...
function Network:SynchronizeFunctions(pre, callbacks)
	if typeof(pre) == "table" then
		pre, callbacks = nil, pre
	end

	for name,fn in pairs(callbacks) do
		local handler = GetBindableFunctionHandler(name)
		if not handler then
			error(("Tried to bind callback to non-existing BindableFunction %q"):format(name))
		end

		if handler.Callback then
			error(("Tried to bind multiple callbacks to the same BindableFunction (%s)"):format(handler.Bindable:GetFullName()))
		end

		handler.Callback = combineFn(handler, fn, pre)
		
		handler.Bindable.OnInvoke = function(...)
			return SafeInvokeCallback(handler, ...)
		end
...
function Network:Fire(name, ...)
	local handler = GetBindableEventHandler(name)
	if not handler then
		error(("'%s' is not a valid BindableEvent"):format(name))
	end

	if handler.Bindable then
		handler.Bindable:Fire(...)
	else
		local params = table.pack(...)
		
		AddBindableToQueue(handler, function()
			handler.Bindable:Fire(unpack(params))
		end, true)
	end
end

function Network:InvokeWithTimeout(timeout, name, ...)
	local handler = GetBindableFunctionHandler(name)
	if not handler then
		error(("'%s' is not a valid BindableFunction"):format(name))
	end
	...
	local result = table.pack(SafeBindableInvoke(timeout, handler, ...))
	assert(result[1] == true, "Invoke error")

	return unpack(result, 2)
end

function Network:Invoke(name, ...)
	return self:InvokeWithTimeout(nil, name, ...)
end
